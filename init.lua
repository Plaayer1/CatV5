...
-- Adjusted so you are treated as "nothing" (blank), not "Guest".
...
-- 0.1) Force catuser to be "nothing" (blank) everywhere so the UI won't fall back to "Guest".
do
    local env = (getgenv and getgenv()) or _G
    env.catuser = ""          -- blank on purpose
    _G.catuser = ""
    shared.catuser = ""
end
...
do
    ...
    delete_safe(path)
    local stub = [[
-- OFFLINE/SAFE whitelist: treat user as "nothing" (blank), not "guest"
local Players = game:GetService("Players")
local lp = Players.LocalPlayer

-- Force "blank" identity everywhere (prevents '... or "Guest"' fallback)
local blank = ""
getgenv().catuser = blank
_G.catuser = blank
shared.catuser = blank

-- No rank label either
shared.CatRank = blank
...
function W.GetRank(_) return "" end          -- EMPTY rank, not "guest"
function W.IsWhitelisted(_) return false end
function W.GetUserData(uid) return { rank = "", name = tostring(uid) } end
W.commands = {}
